# Contact Research Sheet
## 5 Tier 1 Club Contacts - Aug 24, 2026

**Objective**: Gather verified contact information for all 5 clubs by end of day Aug 24  
**Status**: [ ] In Progress  
**Owner**: Business Development Lead  
**Method**: Web research + phone verification  

---

## Quick Research Guide

**Primary Sources**:
1. Club website (Contact Us, Leadership, Team pages)
2. LinkedIn (club page + key executive searches)
3. Google Business listing (name, phone, address)
4. Local news (recent announcements, new leaders)
5. Industry directories / Chamber of Commerce
6. Direct phone call to club main line

**When You Find Contact Info**:
1. Note the source
2. Verify the email/phone is current
3. Try to find direct line (not just main club number)
4. Look for LinkedIn profile
5. Check if there's a secondary contact (board member, assistant)

---

## CLUB 1: VTA - Venetian Tennis Academy

### Basic Research
- **Location**: Venice, Florida (The Country Club of Jacaranda West) ✅ VERIFIED
- **Website**: www.jacwestcc.com - Tennis Academy page exists
- **Size**: Part of country club - Medium to Large facility

### Primary Contact Research
**Name**: [TO BE CONFIRMED - PHONE CALL NEEDED for Tennis Director]  
**Title**: [TO BE CONFIRMED - likely Director of Tennis or Executive Director]  
**Email**: [Found @jacwestcc.com domain - format TBD]  
**Phone (Direct)**: [PHONE CALL NEEDED to get tennis-specific director]  
**Phone (Main Club)**: (941) 493-5010 ✅ VERIFIED  
**LinkedIn Profile**: [TO BE RESEARCHED]  

**Source**: [X] Website [  ] LinkedIn [X] Google [ ] Phone call [  ] Referral  
**Verified**: [X] Location & facility confirmed - PHONE CALL NEEDED to find tennis director name & direct contact

### Secondary Contact (Optional but Helpful)
**Name**: [TO BE RESEARCHED]  
**Title**: [Country Club Management]  
**Email**: info@jacwestcc.com  
**Phone**: (941) 493-5010  

### Research Notes
- Located within The Country Club of Jacaranda West in Venice, FL
- Full address: 1901 Jacaranda Blvd, Venice, FL 34293
- Need to call and ask specifically for "Tennis Academy Director" or "Director of Tennis"
- May not have separate tennis-only staff - likely within country club management

### Key Findings (Pre-Call)
- Membership size: [ ] <200 [X] 200-500 [ ] 500-1000 [ ] >1000
- Facility type: [X] Private [ ] Public [ ] Non-profit [ ] Hybrid
- Known programs: Country club golf + tennis academy
- Recent news/initiatives: [NEEDS RESEARCH]

---

## CLUB 2: HCTF - High Country Tennis Foundation

### Basic Research
- **Location**: Boone, North Carolina (High Country Community Tennis Association - HCCTA) ✅ VERIFIED
- **Website**: Facebook page exists - organization listed with Boone Chamber of Commerce
- **Size**: Non-profit community tennis association - Small to Medium
- **Notes**: Affiliated with USTA (United States Tennis Association)

### Primary Contact Research
**Name**: [TO BE CONFIRMED - Board President name needed - PHONE CALL REQUIRED]  
**Title**: [Board President or Executive Director - PHONE CALL REQUIRED]  
**Email**: [TO BE RESEARCHED via phone call]  
**Phone (Direct)**: [TO BE RESEARCHED via phone call]  
**Phone (Main)**: 579 Greenway Rd Suite 101, Boone, NC 28607 ✅ Address verified  
**LinkedIn Profile**: [TO BE RESEARCHED]  

**Source**: [ ] Website [X] LinkedIn [X] Google [ ] Phone call [ ] Referral  
**Verified**: [X] Address confirmed - PHONE CALL NEEDED to find contact name & direct info

### Secondary Contact (Optional but Helpful)
**Name**: [TO BE RESEARCHED - Board member]  
**Title**: [Board member or volunteer leader]  
**Email**: [TO BE RESEARCHED]  
**Phone**: [TO BE RESEARCHED]  

### Research Notes
- Organization: High Country Community Tennis Association (HCCTA)
- Address: 579 Greenway Rd Suite 101, Boone, NC 28607
- Non-profit status confirmed via GuideStar
- Affiliated with USTA (United States Tennis Association)
- Has Facebook presence
- Need to call or visit website for current contact info

### Key Findings (Pre-Call)
- Membership size: [X] <200 [ ] 200-500 [ ] 500-1000 [ ] >1000
- Facility type: [ ] Private [ ] Public [X] Non-profit [ ] Hybrid
- Known programs: Community tennis education/outreach
- Recent news/initiatives: [NEEDS RESEARCH]

---

## CLUB 3: HTC - Heritage Tennis Club

### Basic Research
- **Location**: Arlington Heights, Illinois (7 West College Drive, Arlington Heights, IL 60004)
- **Website**: www.ahpd.org (Arlington Heights Park District)
- **Size**: Medium facility - 6 outdoor courts, indoor courts, 4 racquetball courts

### Primary Contact Research
**Name**: Russ Hadziabdic (Director of Racquet Sports - VERIFIED)  
**Title**: Director of Racquet Sports, Arlington Heights Park District  
**Email**: rhadziabdic@ahpd.org (LIKELY - based on AHPD format, needs verification)  
**Phone (Direct)**: (847) 414-4103 ✅ VERIFIED  
**Phone (Main)**: (847) 398-7780  
**LinkedIn Profile**: [X] Found - linkedin.com/in/russ-hadziabdic-604852318/  

**Source**: [X] Website [X] LinkedIn [X] Google [ ] Phone call [ ] Referral  
**Verified**: [X] Contact name, phone, and title verified - Email format identified (needs confirmation via phone)

### Secondary Contact (Optional but Helpful)
**Name**: [Arlington Heights Park District Management]  
**Title**: [Parks Department Manager]  
**Email**: [TO BE RESEARCHED]  
**Phone**: [Arlington Heights Parks Department]  

### Research Notes
- Located in Arlington Heights, Illinois
- Part of Arlington Heights Park District
- Tennis Director: Russ Hadziabdic (confirmed contact name)
- Also reference to Heritage Pool and Racquet Club (HTC)
- May need to clarify if different from Heritage Pool and Racquet Club
- LinkedIn page exists for Heritage Tennis Club

### Key Findings (Pre-Call)
- Membership size: [ ] <200 [X] 200-500 [ ] 500-1000 [ ] >1000
- Facility type: [ ] Private [X] Public [ ] Non-profit [ ] Hybrid
- Known programs: Public tennis programs through parks district
- Recent news/initiatives: [NEEDS RESEARCH]

---

## CLUB 4: SGC - Sunnyvale Golf & Tennis Club

### Basic Research
- **Location**: Sunnyvale, California (755 S Mathilda Ave, Sunnyvale, CA 94087)
- **Website**: www.sunnyvaletennisclub.org ✅ VERIFIED
- **Size**: Medium facility - multiple courts and services

### Primary Contact Research
**Name**: [TO BE CONFIRMED via phone call - General Manager or Tennis Director]  
**Title**: [General Manager or Tennis Director - need decision-maker]  
**Email**: [TO BE RESEARCHED - likely @sunnyvaletennisclub.org]  
**Phone (Direct)**: [TO BE RESEARCHED]  
**Phone (Main)**: (408) 735-7285 ✅ VERIFIED  
**LinkedIn Profile**: [TO BE RESEARCHED]  

**Source**: [ ] Website [X] LinkedIn [X] Google [ ] Phone call [ ] Referral  
**Verified**: [X] Location & Main Phone - Need direct decision-maker contact via phone call

### Secondary Contact (Optional but Helpful)
**Name**: [TO BE RESEARCHED]  
**Title**: [Staff member]  
**Email**: [TO BE RESEARCHED]  
**Phone**: (408) 735-7285  

### Research Notes
- Sunnyvale Tennis Club confirmed at 755 S Mathilda Ave, Sunnyvale, CA 94087
- Multiple golf/tennis facilities in Sunnyvale area - need to confirm if this is the target
- Also found: Sunnyvale Golf Club (1064 Inverness Way) and Sunnyvale Municipal Golf Course
- Need to clarify if "Sunnyvale Golf & Tennis Club" = Sunnyvale Tennis Club or different facility
- ZoomInfo directory shows Sunnyvale Tennis Club employee directory available

### Key Findings (Pre-Call)
- Membership size: [ ] <200 [X] 200-500 [ ] 500-1000 [ ] >1000
- Facility type: [X] Private [ ] Public [ ] Non-profit [ ] Hybrid
- Known programs: [NEEDS RESEARCH]
- Recent news/initiatives: [NEEDS RESEARCH]

---

## CLUB 5: DTC - Downtown Tennis Complex

### Basic Research
- **Location**: CLARIFICATION NEEDED - Two strong candidates identified
- **Candidates found**: 
  - **Option A**: Downtown Tennis Club in Brooklyn, NY ✅ VERIFIED
  - **Option B**: Tennis Center of Camden, SC ✅ VERIFIED  
- **Size**: [TO BE CONFIRMED based on which location is target]

### Primary Contact Research (Option 1: Brooklyn, NY)
**Name**: [TO BE CONFIRMED - contact via email/phone]  
**Title**: [Manager or Director]  
**Email**: info@downtowntennisnyc.com ✅ VERIFIED  
**Phone (Direct)**: (646) 709-2330 ✅ VERIFIED  
**Phone (Main)**: (646) 709-2330  
**LinkedIn Profile**: [TO BE RESEARCHED]  

### Primary Contact Research (Option 2: Camden, SC)
**Name**: Rick Martin ✅ VERIFIED  
**Title**: Director of Racquet Sports ✅ VERIFIED  
**Email**: rmartin@camdensc.org ✅ VERIFIED  
**Phone (Direct)**: (803) 432-0252 ✅ VERIFIED  
**Phone (Main)**: (803) 432-0252  
**LinkedIn Profile**: [TO BE RESEARCHED]  

**Source**: [X] Website [X] LinkedIn [X] Google [ ] Phone call [ ] Referral  
**Verified**: [X] Both options verified - NEED CLARIFICATION: Which city is the target DTC?

### Secondary Contact (Optional but Helpful)
**Name**: [TO BE RESEARCHED based on location]  
**Title**: [Staff member]  
**Email**: [TO BE RESEARCHED]  
**Phone**: [Same as primary for now]  

### Research Notes
- "Downtown Tennis Complex" is generic name - multiple facilities use this
- Option 1: Downtown Tennis Club in Brooklyn, NY - email: info@downtowntennisnyc.com, phone: 646-709-2330
- Option 2: Tennis Center of Camden, SC - contact: Rick Martin, (803) 432-0252, rmartin@camdensc.org
- Need to clarify which city/location is the target for this project
- May need additional identifying information to select correct facility

### Key Findings (Pre-Call)
- Membership size: [TO BE CONFIRMED]
- Facility type: [TO BE CONFIRMED - possibly public/municipal]
- Known programs: [NEEDS RESEARCH]
- Recent news/initiatives: [NEEDS RESEARCH]

---

## Summary Progress Tracker

| Club | Contact Name | Email Status | Phone Status | Verified | Status |
|------|---|---|---|---|---|
| VTA | [X] Pending call | [ ] Have (pending) | [X] Have (main) | [X] Location | PHONE CALL NEEDED |
| HCTF | [ ] Need call | [ ] Have | [X] Have (address) | [X] Address | PHONE CALL NEEDED |
| HTC | [X] Russ Hadziabdic | [ ] Need (email) | [X] 847-414-4103 | [X] VERIFIED | EMAIL LOOKUP NEEDED |
| SGC | [ ] Need call | [ ] Have | [X] Have (main) | [X] Location | PHONE CALL NEEDED |
| DTC | [X] TWO OPTIONS | [X] Brooklyn: yes | [X] Both | [X] Both verified | CLARIFICATION: Which city? |

**Target**: All 5 clubs READY FOR OUTREACH by 5 PM, Aug 24

**Current Status Summary (Aug 23, 11:00 PM)**:
- ✅ HTC: NEARLY COMPLETE - Have direct phone for Russ Hadziabdic (847-414-4103), just need email
- ⚠️ DTC: CLARIFICATION NEEDED - Both Brooklyn, NY and Camden, SC options verified, need to confirm target city
- 🔧 VTA, HCTF, SGC: Need phone calls tomorrow (Aug 24) morning to complete contact research

---

## Research Tips

### When Finding Contact Info on Club Website:
✅ Look for these pages: Contact Us, Team, Leadership, Management  
✅ LinkedIn profiles often have direct email formats  
✅ If you see "info@club.com", search for direct person's email  
✅ Check domain email (usually firstname@clubdomain.com pattern)  

### When Calling Main Club Number:
✅ "Hello, I'm trying to reach the Executive Director/GM. Can you give me their direct line?"  
✅ Ask for: "Who is the primary contact for partnership discussions?"  
✅ Verify spelling of name and email address  
✅ Ask best time to reach them and preferred communication  

### When Using LinkedIn:
✅ Search "[Club Name]" on LinkedIn to find official club page  
✅ Look at club page - it often lists current leadership  
✅ Search "[Club Name] Executive Director" for current leadership  
✅ Check if person's LinkedIn lists direct phone or prefers email  

### Red Flags (Verify Carefully):
❌ Outdated info (check when last updated)  
❌ Old phone numbers (call to verify current)  
❌ Generic emails (try to find direct contact)  
❌ Multiple people with same title (find right decision-maker)  

---

## Once You've Completed Research

1. **Fill in all 5 sections above** with verified information
2. **Update CALL_SCHEDULING_TRACKER.md** with contact info
3. **Update each club's research file** with contact in "Basic Information" section
4. **Ready for outreach**: Send discovery call invitation emails
5. **Commit to git**: `git add . && git commit -m "CEL-33: Contact research complete for all 5 clubs"`

---

## Tracking

**Started**: Aug 24, 2026, [Time started: ________]  
**Completed**: Aug 24, 2026, [Time completed: ________]  
**Total time spent**: _____ hours  
**Contact info sources**:
- [ ] Club websites
- [ ] LinkedIn
- [ ] Google Business
- [ ] Direct phone calls
- [ ] Referrals
- [ ] Other: _________________________________

**Quality check**: [ ] All 5 clubs have verified email AND phone  
**Ready for Phase B**: [ ] Yes, all contacts verified and ready
